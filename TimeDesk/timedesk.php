<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="timedesk.css">

    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.2.0/css/all.min.css" integrity="sha512-xh6O/CkQoPOWDdYTDqeRdPCVd1SpvCA9XXcUnZS2FmJNp1coAFzvtCN9BmamE+4aHK8yyUHUSCcJHgXloTyT2A==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    <title>Portal</title>
</head>
<body>
    <div id="pre-loader">
        <h1 style="text-align: center; margin-top: 15vh">Please Wait!......</h1>
    </div>
    <div class="section-1">
        <div class="navbar">
            <div class="header-logo">
                <img src="logo_MPR.png" alt="" width="90" height="90">
                <h1 style="margin-top: 3%;"><a href="index.html" style="text-decoration: none; ">TimeDesk</a></h1>
                <!--<h1>Portal</h1>--->
            </div>
            <div class="header-tabs" style="margin-left: 27%;">
                <!-- <a href="#" id="#aa">Notices </a> -->
                <a href="login.php">Admin </a>
                <a href="login.php">Attendence </a>
                
            </div>
            <div class="header-login"style="margin-left:-13%;" >
                <a href="login.php">Login</a>
               
            </div>
        </div>
    </div>
    <div class="section-2">
        <div class="curve">
            <img src="backimagcurves-removebg-preview.png" alt="" width="1518" height="700" >
        </div>
        <div class="divbox">
        <div class="head-box">
            <h1>Accelerate Education, <br>Achieve Success.</h1>
            <p>With detailed reports and data representation<br> makes education simpler for everyone!</p>
            
        </div>
        <div class="image-box">
            <img src="Deskfinal-removebg-preview.png" alt="" width="750" height="500">
        </div>
    </div>  
    </div>
    <div class="section-3">
        <div class="curve3">
            <img src="Desk-final-white.png" alt="" width="1518" height="700">
        </div>
        <div class="content-box">
            <div class="content-box-1">
               
                <button type= "button" id="Box1" style="background-color: #454ea0; " >NOTICE<br>Coming Soon!</button>
            
                <A href="login.php"><button  id="Box2" style="background-color:#f7bc52 ;">ADMIN</button></A>
                
            </div>
            <div class="content-box-2">
                <button id="Box3" style="background-color:#f7bc52 ;">STUDENT<br>Coming Soon!</button>
                <A href="login.php"><button id="Box4" style="background-color: #454ea0;">ATTENDANCE</button></A>
            </div>
            </div>
            <div class="writediv">
            <div class="writeup">
                <p style="text-transform:uppercase ; line-height: 0.5rem;">TimeDesk Makes your life easy</p>
                <h1 style="line-height: 4.5rem;">All in One Portal</h1>
                <hr width="450px">
                <p style="margin-top: 25px; margin-bottom: 50px">Lorem ipsum dolor sit amet consectetur adipisicing elit. Ad, nihil earum asperiores possimus unde et? Quisquam aperiam deleniti, minus mollitia, explicabo iste at ducimus veritatis vitae adipisci ad aliquid quidem!</p>
                <a  href="login.php" style="text-decoration:none;"class="btn">View More</a>
            </div>
        </div>
        
        
    </div>
    
    <div id="section4">
        <div>
            <h1 id="perfect">Perfect Tools For Teachers</h1>
            <p id="perfectpara">Lorem ipsum is placeholder text commonly used in the graphic, print,<br> and publishing industries for previewing layouts and visual mockups.</p>
            <button id="learnmore"><a href="login.php">LEARN MORE</a></button>
        </div>
        
        <img id="teacherstudent" src="teacherandstudent.jpg"></img>
    </div>
    <div id="section5">

        <img id="teacherstudent1" src="graph.jpg"></img>

        <div>
            <h1 id="perfect">Manage Your Learning!</h1>
            <p id="perfectpara">Lorem ipsum is placeholder text commonly used in the graphic, print,<br> and publishing industries for previewing layouts and visual mockups.</p>
            <button id="learnmore" onclick="about()"><a href="login.php">LEARN MORE</a></button>
        </div>
        
    </div>
    
<!--Section-5-->
    <div class="getstarted">
        <h1>Accelerate your learning!</h1>
        <div class="getstartedbuttons" style="width:200px;">
            <a href="login.php" ><button>GET STARTED</button></a>
            
        </div>
    </div>

    
    <script src="timedesk.js"></script>
    <script src="main.js"></script>
    
</body>
</html>